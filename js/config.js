...
const CONFIG = {
  ...
  /* ── 结局2「回返」：删除之后又回来 ──────────────────
     按下独裁者按钮、泡泡真的清空了，然后同一个烦恼一次一次回来。
     空白那 2.5 秒是整段演出的地基：没有那一拍真空，后面的「又回来了」
     就只是「没清干净」，不是「确实消失过」。重复次数宁可少不可多：
     到第六次玩家已经知道结果了，再加就是在磨人。 */
  ENDING2_REPEAT_CLICKS: 6,
  ENDING2_BLANK_MS: 2500,
  ENDING2_RESPAWN_MS: 900,
  ENDING2_GROUP_SPAWN_MS: 420,
  ENDING2_SETTLE_MS: 2600,
  // 最后一批回来的数量区间。比本轮实际烦恼数多一点，
  // 但不能多到把袋子填满——填满就回到了关卡，不是结局。
  ENDING2_FINAL_MIN: 5,
  ENDING2_FINAL_MAX: 8,

  /* ── 结局3「暂时晴朗」：一步一步亲手处理完 ──────────────
     旧判定是「亲手点满 target=60 颗」。第三关共 26 秒，要求平均
     2.3 次/秒且全程零逃逸零自爆——实际上打不出来，结局3 形同虚设。
     现在改成看结果而不是看计数：**场上活着的泡泡清零**，且一颗没逃、
     一颗没自爆。生成在倒计时剩 4.2 秒时停住（update 里的 finalStop），
     所以“把剩下的清完”是一个真实可达的目标，而不再要求恰好点满 60 次。
     MIN_CLEARED 是防误判：开局铺底 12 颗，没清到这个数不算「处理完了」。
     容差两项现在都是 0（严格）。实测下来如果还是过于苛刻，
     改这两个数字就行，不用动逻辑。 */
  ENDING3_MIN_CLEARED: 12,
  ENDING3_MAX_ESCAPED: 0,
  ENDING3_MAX_AUTO_BURST: 0,
  /* 收束演出的四个时间点（从最后一颗消散算起）：
     先留一秒安静让玩家确认场上真的空了，再退 HUD，
     然后按钮熄灭，最后空间舒展。四个点必须递增。 */
  ENDING3_CALM_MS: 1000,
  ENDING3_BUTTON_OFF_MS: 2550,
  ENDING3_SPACE_MS: 3500,
  ENDING3_ENTER_MS: 4600,

  /* ── 结局4「过载」：因果全部发生在袋内 ────────────────
     袋子不会破、泡泡不会从扎口逃走——那是结局1 的语汇。
     前 1.9 秒仍可点击是整段的关键：要让玩家亲身感到「越处理越赶不上」，
     而不是看一段演出。SPAWN_*_MS 比关卡里任何一档都快。 */
  ENDING4_EFFORT_MS: 1900,
  ENDING4_CROWD_MS: 1750,
  ENDING4_PULSE_MS: 2200,
  ENDING4_FIRST_PAUSE_MS: 430,
  ENDING4_CHAIN_MS: 1450,
  // 连锁之后那一拍强制静止。短，但不能没有：
  // 高密度动势必须「突然断掉」，渐停会变成普通的收尾。
  ENDING4_STILL_MS: 420,
  ENDING4_SETTLE_MS: 2600,
  ENDING4_MAX_BUBBLES: 34,
  ENDING4_SPAWN_EFFORT_MS: 140,
  ENDING4_SPAWN_CROWD_MS: 120,
  // 拥挤度到这个值就提前进入波动段，不必等满 CROWD_MS。
  ENDING4_PACKING_TRIGGER: 0.84,

  /* ── 音频：7 首 BGM + 17 条音效 ────────────────────────────
     素材在 assets/audio/{bgm,sfx}/，由 assets/dev/_build-audio.py 从母带转出，
     每条都有 .opus 与 .m4a 两份，运行时按 canPlayType 选一种。
     搭配关系来自《独裁者按钮_音效使用与BGM搭配表.xlsx》。

     母带电平跨度 23dB（SFX12 峰值 −0.6dB，SFX08 平均 −38.4dB），
     转码时已经统一归到 −3dBFS 峰值。也就是说下面的 TRIM **不是**在补电平差，
     而是在配「这条音效在这个场景里该占多大分量」——归一化让它们站上同一条线，
     这张表决定谁站前面。改音量优先动这里，不要回去改转码脚本。 */

  // 总音量 × 分类音量 × 逐条修正 × 调用点的临时系数 = 最终 volume。
  // BGM 压到 0.44 是因为它全程在响，而音效是点状的：等响的话音效会被糊住。
  AUDIO_MASTER_GAIN: 0.9,
  AUDIO_BGM_GAIN: 0.44,
  AUDIO_SFX_GAIN: 0.85,

  // BGM 交叉淡入淡出。切场景很频繁，硬切会「啪」一下。
  AUDIO_BGM_FADE_MS: 900,
  // 淡入淡出的推进步长。40ms = 25Hz，人耳听不出台阶，也不至于压垮主线程。
  AUDIO_FADE_STEP_MS: 40,

  // duck：老虎机滚动、独裁者按钮的 stinger 期间把底乐压到这个倍率，
  // 让前景音效站出来。压底乐比调大前景安全——调大会削顶。
  AUDIO_DUCK_RATIO: 0.28,
  AUDIO_DUCK_MS: 320,

  // 每条音效的元素池大小。连点泡泡时同一条音效要能叠着响，
  // 一个 <audio> 元素同一时刻只播得了一遍。4 个够用：
  // 再多也只是让更早的那一声拖在后面，听起来是糊的。
  AUDIO_SFX_POOL_SIZE: 4,

  /* 逐条音量修正。1.0 是基准（= MASTER × SFX_GAIN）。
     依据是表格里的「使用建议」那一列：
       · SFX01 全站基础反馈，「音量较轻，避免盖过BGM」→ 压到 0.55
       · SFX02 悬停，比点击更轻一档，不然扫过一排卡片会很吵
       · SFX09/11/12 是三关里最高频的三条，必须压住，否则满屏泡泡时全是它们
       · SFX07/08 奖励音，表格要求「保持克制，不做手游式夸张反馈」
       · SFX14 倒计时是压力信号，允许它突出来一点
       · SFX15/16 泡泡重新出现、SFX17 按钮熄灭：都是安静场景里的单点声，
       前后没有别的声音抢位置，所以不需要压，保持基准附近 */
  AUDIO_SFX_GAIN_TRIM: Object.freeze({
    sfx01: 0.55, sfx02: 0.42, sfx03: 0.70, sfx04: 0.85, sfx05: 0.60,
    sfx06: 0.75, sfx07: 0.72, sfx08: 0.66, sfx09: 0.48, sfx10: 0.70,
    sfx11: 0.62, sfx12: 0.62, sfx13: 0.80, sfx14: 0.88, sfx15: 0.70,
    sfx16: 0.70, sfx17: 0.70
  }),

  /* 冷却窗口（毫秒）：同一条音效在窗口内的重复调用直接丢弃。
     表格原话：「高频事件，需要设置播放冷却/限频，避免大量泡泡同时发声」。
     第三关的生成间隔是 210ms，所以 sfx09 的冷却必须大于它——
     否则每一颗新泡泡都响一声，听起来就是一片噪声而不是「又冒出来一个」。
     点击音的冷却要小得多：那是玩家自己的动作，压掉了会觉得没反应。 */
  AUDIO_SFX_COOLDOWN_MS: Object.freeze({
    sfx01: 60, sfx02: 140, sfx03: 200, sfx06: 40,
    sfx09: 320, sfx11: 70, sfx12: 70, sfx13: 900,
    // 结局4 的连锁破裂一秒内会炸十几颗，泡泡声必须限频，
    // 否则那一段就是一片噪声，听不出「啦…啦…啦啦」的传播感。
    sfx15: 260, sfx16: 260
  }),

  // 泡泡消除音随机池。表格：「两者随机交替，降低重复感」。
  // 结局4 也暂用这两条（表格「结局4：可暂用SFX11/12」）。
  AUDIO_BUBBLE_POP_KEYS: Object.freeze(['sfx11', 'sfx12']),

  /* 场景 → BGM。表格给的是「网站阶段」，这里落到 u01~u12：
       BGM1 体验馆/道具浏览 · BGM2 烦恼选择/道具匹配 · BGM3 三关游戏
       BGM4 第三关压力增强 · BGM6 星级/收藏奖励 · BGM7 四种结局
     BGM5「独裁者按钮」不在这张表里：它只有 12 秒，不是可循环的底乐，
     而是按钮按下时的一次性 stinger，见 AUDIO_DICTATOR_STINGER。
     u07/u09 是第一、二关的结算节点，整段都走 BGM6（含结算卡、抽卡、收藏册）。 */
  AUDIO_SCENE_BGM: Object.freeze({
    u01: 'bgm1', u02: 'bgm1',
    u03: 'bgm2', u04: 'bgm2', u05: 'bgm2',
    u06: 'bgm3', u07: 'bgm6', u08: 'bgm3', u09: 'bgm6', u10: 'bgm4',
    u11: 'bgm7', u12: 'bgm7'
  }),

  // 独裁者按钮按下时的一次性 stinger。播完自动恢复底乐。
  AUDIO_DICTATOR_STINGER: 'bgm5',

  // 结局2「回返」：泡泡重新出现。两条随机交替，降低重复感——
  // 同一颗烦恼回来六次，声音一模一样会变成提示音，而不是「又来了」。
  AUDIO_RETURN_BUBBLE_KEYS: Object.freeze(['sfx15', 'sfx16']),

  // 结局3「暂时晴朗」：独裁者按钮熄灭。配合 CSS 的降饱和，
  // 声音在前、画面在后——先听到它安静下来，再看到它暗下去。
  AUDIO_BUTTON_OFF_SFX: 'sfx17',

  /* 触发 SFX03「页面出现」的场景切换白名单。
     表格：「用于明显的场景转换；小提示框无需每次播放」。
     所以只留真正换了一个世界的那几步：进入引导、进入选择、看到匹配结果、
     走进结局、走进体验记录。u06↔u07↔u08↔u09↔u10 之间共用同一块 canvas，
     画面是连续的，响一声反而会把连续感打断。 */
  AUDIO_SCENE_ENTER_SFX: Object.freeze(['u02', 'u03', 'u05', 'u11', 'u12']),

  /* 倒计时音的触发秒数。素材 SFX14 裁完是 4.00 秒，
     所以在剩 4 秒那一刻放一次，正好铺到 0。
     表格说「建议只用于最后5～10秒」——真要铺满 10 秒得循环两次半，
     那条素材不是为循环做的，接缝会很明显，所以按素材的实际长度来。
     注意 onTime 是每秒一次的回调，必须取跳变沿，不然每秒重放一次。 */
  AUDIO_TIME_LOW_AT_SEC: 4,

  /* 麻袋膨胀音只在升档时响（0→1→2→3），降档不响。
     updatePressure 的档位是双向的：玩家清掉泡泡会降回去，
     一路响下来就变成了「袋子一鼓一瘪」的滑稽效果。 */
  AUDIO_BAG_STRESS_MIN_STAGE: 1,

  /* 悬停音的作用范围。表格：「只用于重要卡片，不要给所有文字和普通按钮都添加Hover音」。
     所以是白名单而不是黑名单——选择器写在这里，audio 的委托只认这几类。
     同一个元素只在**第一次**移入时响（表格：「鼠标第一次移入」）。 */
  AUDIO_HOVER_SELECTOR: '[data-action="pick-worry"], [data-action="pick-category"], ' +
    '[data-action="show-gadget-tip"], .collection__cell.is-unlocked',

  // 结局1「远去」的整体漂浮环境音（SFX10）。
  // 表格：「作为整体环境层使用，不要绑定到每一颗泡泡」。
  // 淡入要慢：泡泡是一颗颗松开的，声音突然出现会比画面早一拍。
  AUDIO_FAREWELL_FADE_MS: 1400
};
