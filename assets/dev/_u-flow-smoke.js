...
async function run() {
  ...
  const reelCells = await page.locator('[data-bind="reelA"] .slot__cell').count();
  if (reelCells !== 21) fail('滚轮应为 20 道具 + 1 空位 = 21 格，实际 ' + reelCells);
  else pass('滚轮 21 格');
  ...
  else pass('匹配到道具：' + match.name + '（' + match.group + '）');
  ...
  const afterRestart = await page.evaluate(function () {
    return {
      ...
      selected: Boolean(App.data.selectedWorry),
      bubbles: BubbleGame.getBubbleCount()
    };
  });
